<x-guest-layout>
            ...
            <div class="md:flex w-full">
                ...
                <div class="w-full md:w-1/2 py-10 px-5 md:px-10">
                    <div class="text-center mb-10">
                        <h1 class="font-bold text-3xl text-gray-900">Hora da Selfie!</h1>
                        <p>Não precisa se preocupar em tirar uma foto bonita, mas temos umas dicas para você.</p>
                    </div>
                    <div>
                        <div class="flex -mx-3">
                            <div class="w-1/2 px-3 mb-5">
                                {{--                                <label for="" class="text-xs font-semibold px-1">First name</label>--}}
                                <label for="" class="text-xs font-semibold px-1"> <img
                                        src="{{ asset('images/cap.jpg') }}" alt=""></label>
                                <div class="">
                                    <label for="">Acessórios não permitidos:</label>
                                    <ul>
                                        <li>Boné</li>
                                        <li>Fones de ouvido</li>
                                        <li>Óculos de grau ou <br> óculos de sol</li>
                                    </ul>
                                </div>
                            </div>
                            <div class="w-1/2 px-3 mb-5">
                                <label for="" class="text-xs font-semibold px-1"> <img
                                        src="{{ asset('images/self.jpg') }}" alt=""></label>
                        ...
                        @if(! auth()->user())
                            <form method="POST" action="{{ route('self', ['user' => session()->get('user')->id]) }}"
                        ...
                        @endif
                                ...
                                <div class="flex -mx-3">
                                    <div class="w-full px-3 mb-5">
                                        <label for="self" class="block w-full max-w-xs mx-auto bg-indigo-500 hover:bg-indigo-700
                                focus:bg-indigo-700 text-white rounded-lg px-3 py-3 font-semibold text-center">Tirar
                                            Self</label>
                                        <input type="file" accept="image/*" capture name="self" required class="block w-full max-w-xs mx-auto bg-indigo-500 hover:bg-indigo-700
                                focus:bg-indigo-700 text-white rounded-lg px-3 py-3 font-semibold" id="self">
                                        <span id='file-name'></span>
                                    </div>
                                </div>
                                <div class="flex -mx-3">
                                    <div class="w-full px-3 mb-5">
                                        <button class="block w-full max-w-xs mx-auto bg-indigo-500 hover:bg-indigo-700
                                focus:bg-indigo-700 text-white rounded-lg px-3 py-3 font-semibold"
                                                type="submit">Enviar Self
                                        </button>
                                    </div>
                                </div>
                            </form>
                            @if(is_null(auth()->user()))
                                <a href="{{ route('password.page', ['user' => session()->get('user')->id]) }}" class="block w-full max-w-xs mx-auto bg-indigo-500 hover:bg-indigo-700
                                focus:bg-indigo-700 text-white rounded-lg px-3 py-3 font-semibold text-center"> Tirar
                                    Self
                                    depois
                                </a>
                            @endif
                            @if(auth()->user())
                                <a href="{{ route('dashboard') }}" class="block w-full max-w-xs mx-auto bg-indigo-500 hover:bg-indigo-700
                                focus:bg-indigo-700 text-white rounded-lg px-3 py-3 font-semibold text-center"> Tirar
                                    Self
                                    depois
                                </a>
    ...
    @push('scripts')
        <script src="{{ asset('js/formRegister.js') }}"></script>
    @endpush
